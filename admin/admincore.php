...
if(isset($_GET['act']) && $_GET['act']=='ads' &&isset($_POST['edit']) && $_POST['edit']==1){
	...
	if(file_put_contents('../data/cdkj.ads.php',"<?php\n \$ads =  ".var_export($data,true).";\n?>")){
		$tips = '<span class="red" style="font-size:18px; margin-bottom:15px; display:block;">修改成功！</span>';
	}else{
		$tips = '<span class="red" style="font-size:18px; margin-bottom:15px;display:block;">修改失败！可能是文件权限问题，请给予目录写入权限</span>';
	}	
     $cdkj = $data;
}
...
if( isset($_GET['act']) && $_GET['act']=='setting' && isset($_POST['edit']) && $_POST['edit']==1){
    ...
    if(!isset($datas['cdkj']['joinhotkey'])){
		$data['joinhotkey']='0';
	}
	if($data['admin_pass']==''){
		$data['admin_pass'] = $cdkj['admin_pass'];
	}else{
	    $data['admin_pass'] = md5ff(htmlspecialchars($data['admin_pass']));	
	}
	
	if(file_put_contents('../inc/cdkj.config.php',"<?php\n \$cdkj =  ".var_export($data,true).";\n?>")){
		$tips = '<span class="red" style="font-size:18px; margin-bottom:15px; display:block;">修改成功！</span>';
	}else{
		$tips = '<span class="red" style="font-size:18px; margin-bottom:15px;display:block;">修改失败！可能是文件权限问题，请给予inc/cdkj.config.php写入权限</span>';
	}	
     $cdkj = $data;
} 
 //setmovie
if( isset($_GET['act']) && $_GET['act']=='seturl' && isset($_POST['edit']) && $_POST['edit']==1){	
	...
	if(file_put_contents('../data/cdkj.seturl.php',"<?php\n \$seturl =  ".var_export($seturl,true).";\n?>")){
